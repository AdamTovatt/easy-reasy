...
namespace EasyReasy.Auth
{
    ...
    /// records (successful authentications, failed authentications, second-factor enrollment, session
    /// termination, and bulk session revocation).
    /// </summary>
    /// <remarks>
    ...
    /// <see cref="OnWebAuthnRegistrationAsync"/> goes one step further and is invoked by the consumer only: the WebAuthn
    /// ceremony runs partly in the browser, so this library ships no endpoint for it and
    /// <see cref="WebAuthnVerifier"/> is a pure function that takes neither an <see cref="HttpContext"/> nor a logger.
    /// </para>
    /// <para>
